...
func main() {
    ...
    fields := flag.String("f", "count,txid,vout,value,type,address", "Fields to include in output. [count,txid,vout,height,value,coinbase,nsize,script,type,address]")
    ...
    fieldsAllowed := []string{"count", "txid", "vout", "height", "coinbase", "value", "nsize", "script", "type", "address"}

    // Check that all the given fields are included in the allowedFields array
    for _, v := range strings.Split(*fields, ",") {
        exists := false
        for _, w := range fieldsAllowed {
            if v == w { // check each field against every element in the allowedFields array
                exists = true
            }
        }
        if exists == false {
    ...
    for iter.Next() {
        ...
        if (prefix == 67) { // 67 = 0x43 = C = "utxo"
            ...
            // Value
            value := btcleveldb.DecompressValue(varintDecoded)
            output["value"] = fmt.Sprintf("%d", value)
            totalAmount += value // add to stats
    ...
    fmt.Println()
    fmt.Printf("Total UTXOs: %d\n", i) // convert satoshis to BTC (float with 8 decimal places)
